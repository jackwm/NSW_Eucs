...
place <- regexpr(t, pattern='[[:digit:]]{2}[[:alpha:]]') 
place[[2]] <- attr(place,which='match.length')
# using substring to pull out the 'site' name
out$Site <- substr(x=t, start=place[[1]], stop= (place[[1]]+place[[2]]) )
rm(t)
head(out)
...
z$Tree <- substr(x=t, start=place.port[[1]], stop=place.port[[1]])
z$Site <- substr(x=t, start=place[[1]], stop= (place[[1]]+place[[2]]) )
